...
async function sendSesliCagriKirmiziSantral(phoneNumbers, message) {
    ...
    for (const gsm of phones) {
        ...
        try {
            ...
            if (callinfo?.status === 1) {
                logger.info(`Kırmızı Santral Sesli Çağrı Gönderme Başarılı (${gsm}):`, JSON.stringify(callinfo))
                successIds.push(callinfo.uniqueid)
            } else {
                logger.info(`Kırmızı Santral Sesli Çağrı Gönderme Başarısız (${gsm}):`, JSON.stringify(response.data))
                allSuccess = false
            }
        } catch (err) {
            logger.error(`Kırmızı Santral Sesli Çağrı Gönderme Hatası (${gsm}):` + err)
            allSuccess = false
        }
    }

    if (successIds.length > 0) {
        return { success: allSuccess, data: successIds.join(',') }
    }
    return { success: false, data: 'Başarısız' }
}
...
export { sendSesliCagriNetGsm, sendSesliCagriKirmiziSantral, sendSesliCagriKobicom }
