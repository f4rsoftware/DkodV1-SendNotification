...
async function runSmsTaskList(taskCallId) {
    ...
    while (notificationSendCount !== 0 && counter < settings.bildirimGondermeDenemeSayisi) {
        try {
            ...
            const smsPersonelPhonesKobicom = smsGonderilecekGrup.map(item => item.PERSONEL_TELEFON) //Bildirim Gönderilecek Sms Kayıtların Telefon Numaraları Alınıyor.
            //console.log('Sms Personel Phones Kobicom : ' + smsPersonelPhonesKobicom)
            ...
            if (useFirstFirm) {
                ...
                responseSms = await sendSMSKobicom(smsPersonelPhonesKobicom, mesajDegistir)
                await setNotificationStatus('BILDIRIM_SMS_FIRMA', null, 'KOBICOM',islemeAlinanKayitlar) //Bildirim Gönderilen Firma İşleniyor
