...
async function handleSesliCagriResponse(response, taskCallId, gonderilenNumara, isNetGsm) {
    let logPrefix = isNetGsm ? 'NetGsm' : 'Kobicom'
...
async function runSesliCagriTaskList(taskCallId) {
    ...
    while (notificationSendCount !== 0 && counter < settings.bildirimGondermeDenemeSayisi) {
        try {
            ...
            const sesliCagriPersonelPhonesKobicom = sesliCagriGonderilecekGrup.map(item => item.PERSONEL_TELEFON) //Kobicom Bildirim Gönderilecek Sesli Çağrı Kayıtların Telefon Numaraları Alınıyor.
            ...
            if (useFirstFirm) {
                ...
                //Kobicom İçin Tek Tek Gönderim İşlemi Yapılıyor.
                responseSesliCagri= await sendSesliCagriKobicom(sesliCagriPersonelPhonesKobicom, sesliCagriText,taskCallId)
                await setNotificationStatus('BILDIRIM_SESLI_CAGRI_FIRMA', null, 'KOBICOM',islemeAlinanKayitlar) //Bildirim Gönderilen Firma İşleniyor.
                gonderimTipiNetGsm = false
            }

            //Response Cevaplarını Toplu Olarak Güncelle.
            ...
            const result = await handleSesliCagriResponse(responseSesliCagri, null, islemeAlinanKayitlar, gonderimTipiNetGsm)
